using UnityEngine;

public class CONSTANT
{
    public static string TAG_PLAYER = "Player";
    public static string TAG_OBSTACLE = "Obstacle";
    public static string TAG_CRITTER = "Critter";
    public static string TAG_BULLET = "Bullet";
    public static string TAG_BOSS = "Boss";

    public static string ANIMATION_MOVE_X = "moveX";
    public static string ANIMATION_MOVE_Y = "moveY";
    public static string ANIMATION_BOOSTING = "boosting";
    public static string ANIMATION_BOSS_CHARGING = "charging";

    public static string OBSERVER_PLAYERDEATH = "playerDeath";
    public static string OBSERVER_PLAYERHIT = "playerHit";
    public static string OBSERVER_PLAYERBOOST = "playerBoost";
    public static string OBSERVER_PLAYERLEVELUP = "playerLevelUp";
    public static string OBSERVER_DROP_EXP = "dropExp";
}
